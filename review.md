...
## 추가 검증 (Chrome 확장 연결 후 재검증, 커밋 이후)

최초 커밋(`a03a63b`) 이후 Chrome 브라우저 확장이 연결되어 실제 클릭 기반 재검증을
수행했다. 목록(필터/정렬 클릭), 상세(수량 조절+담기+toast), 장바구니(수량 변경/삭제/합계
실시간 재계산), 결제(검증 실패/성공 케이스, 주문 완료, 빈 장바구니 가드) 전체 플로우를
실제 브라우저 클릭으로 왕복 확인했고, 위에서 코드 검증만으로 확인했던 항목들이 실제
렌더링에서도 전부 정상 동작함을 확인했다.

이 과정에서 **새로운 실물 버그 1건**을 발견해 직접 수정했다:

- **`[hidden]` 속성이 CSS로 무시되는 버그**: `.checkout-complete`(`display: flex`),
  `.checkout-form`(`display: flex`), `.cart-summary`(`display: flex`) 세 클래스가 모두
  `display`를 명시적으로 지정하고 있어, 이 요소들에 `hidden` 속성이 설정되어도 브라우저의
  기본 `[hidden] { display: none }` UA 스타일보다 author 스타일이 우선 적용되어 계속
  보이는 문제였다. 실제로 장바구니에 상품이 있는 상태로 `checkout.html`에 처음 진입하면
  결제 폼과 "주문이 완료되었습니다" 완료 화면이 **동시에** 렌더링되는 것을 스크린샷으로
  확인했다(둘 다 `hidden` 속성 유무와 무관하게 보임). `getComputedStyle`로 확인한 결과
  `#checkout-complete-view`는 `hidden` 속성이 `true`인데도 `display: flex`로 계산되고
  있었다.
  - **수정**: `css/store.css` 최상단에 `[hidden] { display: none !important; }` 규칙을
    추가해 이 클래스가 무엇이든 `hidden` 속성이 항상 우선하도록 고쳤다. 재검증 결과
    `checkout.html` 최초 진입 시 폼만 보이고, 제출 후에는 폼이 사라지고 완료 화면만
    보이며, 빈 장바구니 상태에서는 폼이 숨겨지고 안내 메시지만 보이는 것을 확인했다.
  - 이 버그는 Work 단계의 Node DOM 목(mock) 테스트로는 잡을 수 없었다 — `hidden` 속성
    자체는 정확히 토글되고 있었고(JS 로직은 완전히 정상), CSS의 `display` 우선순위 문제였기
    때문에 실제 브라우저 렌더링 검증에서만 드러났다. SKILL.md의 "정적 코드 리뷰만으로
    렌더링 버그를 못 잡음" 항목에 해당하는 사례로 기록해둔다.

**환경 관찰 (버그 아님)**: 라이트 테마를 명시적으로 선택했을 때(`data-theme="light"`,
`--color-bg`/`--accent` 등 CSS 변수는 전부 올바른 라이트 값으로 계산됨을 확인) 실제
렌더링 색상이 탁한 회색(`rgb(24,26,27)` 배경 / `rgb(232,230,227)` 텍스트)으로 나와
스토어 CSS 버그처럼 보였으나, 스토어와 전혀 무관한 기존 `index.html`에서도 동일하게
재현되었고, 명시적으로 다크 테마를 선택하면 우리 팔레트의 정확한 색상(`#000000` 배경 /
`#eafffe` 텍스트, 시안 글로우)이 그대로 나왔다. 이는 이 브라우저 환경의 강제 다크모드
설정(시스템이 다크 모드일 때 "라이트로 보이는" 페이지를 브라우저가 자체적으로 재색칠하는
기능)으로 판단되며, 코드 수정 대상이 아니다. 일반 사용자의 기본 브라우저 설정에서는
재현되지 않을 것으로 예상된다.

이 추가 수정(`[hidden]` 규칙)은 최초 커밋 이후 별도 커밋으로 반영한다.
